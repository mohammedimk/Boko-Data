...
#     #     """
#     #     Generate (or mock-generate) a dedicated virtual account for `user`.

#     #     Returns a dict with keys: bank_name, account_number, account_name.
#     #     """
#     #     full_name = (user.get_full_name() or user.username).strip()

#     #     if not settings.VTU_USE_MOCK_PROVIDER:
#     #         return cls._live_generate(user, full_name)  # pragma: no cover
...
#         # Seed randomness off the user's id so the same user always gets the
#         # same mock account number if regeneration is ever attempted.
...
#         # Build a friendly, brand-flavoured account name from the user's real name.
...
#     # @classmethod
#     # def _live_generate(cls, user, full_name: str) -> dict:  # pragma: no cover
#     #     """
#     #     Placeholder wiring for a real provider integration. Example shape
#     #     using a generic REST provider - adapt endpoint/payload/response
#     #     parsing to whichever real provider is used in production.
#     #     """
#     #     url = f"{settings.CHEAPDATAHUB_BASE_URL}/virtual-accounts"
...
#         """Realistic-looking mock data plans, priced per network."""
#         catalog = {
#             'MTN': [
...
#         if self.use_mock:
#             plans = {p['plan_id']: p for p in self._mock_data_plans(network)}
#             plan = plans.get(plan_id)
#             if not plan:
#                 return {'success': False, 'message': 'Selected data plan is no longer available.'}
#             return {
#                 'success': True,
#                 'reference': self._mock_reference(),
#                 'message': f"{plan['data_size']} data delivered to {phone_number}.",
#                 'plan': plan,
#             }
#         return self._post('data/buy', {
#             'network': network, 'plan_id': plan_id, 'phone_number': phone_number,
#         })
...
#             }
#         return self._post('airtime/buy', {
#             'network': network, 'phone_number': phone_number,
#             'amount': str(amount), 'type': airtime_type,
#         })
...
#         if self.use_mock:
#             # Deterministic mock customer name from the card number.
...
#         if self.use_mock:
#             bouquets = {b['bouquet_id']: b for b in self._mock_cable_bouquets(provider)}
#             bouquet = bouquets.get(bouquet_id)
#             if not bouquet:
#                 return {'success': False, 'message': 'Selected bouquet is no longer available.'}
#             return {
#                 'success': True,
#                 'reference': self._mock_reference(),
#                 'message': f"{bouquet['name']} subscription activated for card {smartcard_number}.",
#                 'bouquet': bouquet,
#             }
#         return self._post('cable/pay', {
#             'provider': provider, 'smartcard_number': smartcard_number,
#             'bouquet_id': bouquet_id, 'phone_number': phone_number,
#         })
...
#         """
#         Initialize a Paystack transaction. Amount must be passed in Naira;
#         Paystack expects kobo so we multiply by 100 internally.
#         """
...
#         """Create (or fetch) a Paystack customer — required before a DVA can be issued."""
...
#         """Issue a real Dedicated Virtual Account (DVA) tied to a Paystack customer."""
...
class CheapDataHubAPI:
    ...
    def _post(self, endpoint: str, payload: dict) -> dict:
        ...
        try:
            ...
            return {
                'success': response.status_code in (200, 201) and str(raw.get('status', '')).lower() in ('success', 'true', '1'),
    ...
    def _mock_data_plans(network: str) -> list:
        """
        Realistic-looking mock data plans, kept only as a fallback catalog
        for get_data_plans()/ajax display if ServicePlan has no rows yet.
        NOTE: buy_data() no longer validates against this catalog - the
        real plan catalog and pricing now live in core.models.ServicePlan,
        checked in views.py before this API is ever called.
        """
        catalog = {
            'MTN': [
                {'plan_id': 'MTN-500MB-30D', 'name': 'MTN 500MB Monthly', 'data_size': '500MB', 'validity': '30 Days', 'price': 130.00},
    ...
    def buy_data(self, network: str, plan_id: str, phone_number: str) -> dict:
        ...
        views.py BEFORE this method is ever called, so the mock branch here
        only needs to simulate a successful delivery - it must not
        re-validate plan_id against the old _mock_data_plans() catalog,
        since real ServicePlan IDs (from CheapDataHub's plan-ids page) won't
        match those old placeholder IDs.
        """
        if self.use_mock:
            return {
                'success': True,
    ...
    def buy_airtime(self, network: str, phone_number: str, amount, airtime_type: str = 'VTU') -> dict:
        ...
        provider_id_map = {'MTN': 1, 'AIRTEL': 3, 'GLO': 2, '9MOBILE': 4}  # ← replace with real IDs from your dashboard
    ...
    def _mock_cable_bouquets(provider: str) -> list:
        """
        Kept only as a fallback catalog. NOTE: pay_cable() no longer
        validates against this - real bouquet catalog/pricing now live in
        core.models.ServicePlan, checked in views.py.
        """
        catalog = {
            'DSTV': [
                {'bouquet_id': 'DSTV-PADI', 'name': 'DStv Padi', 'price': 4400.00},
    ...
    def pay_cable(self, provider: str, smartcard_number: str, bouquet_id: str, phone_number: str) -> dict:
        ...
        views.py BEFORE this method is ever called - the mock branch here
        only simulates a successful subscription, it does not re-validate
        bouquet_id against the old _mock_cable_bouquets() catalog.
        """
        if self.use_mock:
            return {
                'success': True,
