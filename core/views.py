...
from .models import UserProfile, Transaction, ServicePlan, ServiceCommission, WebAuthnCredential
from .utils import VirtualAccountService, CheapDataHubAPI, PaystackClient
...
def get_rp_id(request):
    """Extract domain without port (e.g. 'localhost' or 'boko-data.onrender.com')."""
    return request.get_host().split(':')[0]
...
@login_required
def webauthn_register_options(request):
    """Step 1: Generate a registration challenge and store it in the session."""
    user = request.user
...
@require_POST
def webauthn_register_verify(request):
    """Step 2: Verify the browser's response and save the credential."""
    try:
        body = json.loads(request.body)
        nickname = body.get('nickname', '').strip() or 'Biometric Key'
        ...
        verification = verify_registration_response(
            credential=json.dumps(credential_payload),
            expected_challenge=expected_challenge_bytes,
            expected_origin=get_origin(request),
            expected_rp_id=get_rp_id(request),
        )
        ...
        logger.exception("WebAuthn registration failed for user=%s", request.user.username)
        return JsonResponse({'success': False, 'message': f'Registration failed: {exc}'}, status=400)
...
@require_GET
def webauthn_login_options(request):
    """Step 1: Generate an authentication challenge for the named user (or discoverable login)."""
    username = request.GET.get('username', '').strip()
    user = User.objects.filter(username=username).first() if username else None
...
@require_POST
def webauthn_login_verify(request):
    """Step 2: Verify the browser's signed assertion and log the matching user in."""
